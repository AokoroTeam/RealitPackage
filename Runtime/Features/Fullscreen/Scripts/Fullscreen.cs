...
namespace Realit.Core.Features.Fullscreen
{
    ...
    public class Fullscreen : Feature
    {
        ...
        private void OnFullScreenChange()
        {
            if (IsFullScreen)
            {
                ...
                if (FeaturesManager.UI.TryGetUIForFeature(this, out FeatureIndicator icon))
                    icon.SetIcon((Data as FullscreenData).shrink);
            }
            else
            {
                OnFullScreenExit?.Invoke();
                if (FeaturesManager.UI.TryGetUIForFeature(this, out FeatureIndicator icon))
                    icon.SetIcon((Data as FullscreenData).expand);
            }
        }
#if UNITY_WEBGL
        ...
        {
            //LogMessage("Fullscreen feature entered");
            if(!IsFullScreen)
            {
                RequestFullScreen();
                //LogMessage("Requesting fullscreen");
            }
        ...
        {
            //LogMessage("Fullscreen feature exited");
            if (IsFullScreen)
            {
                //LogMessage("Requesting exitting fullscreen");
                DisableFullScreen();
            }
        }
